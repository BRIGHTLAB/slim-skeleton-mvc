...
class PDOMapper {
    ...
    private $reset_token_column = "reset_token";
    private $columns = [];
    ...
    public function insert (Array $model_assoc, $token = null) {
        ...
        if (isset($model_assoc['password'])) {
            ...
            unset($prepare_statement[":repassword"]);
            unset($prepare_statement[":terms-conditions"]);
            unset($prepare_statement[":location_id"]);
        }
    ...
    public function getUserBasedOnResetToken ($token) {
        $sql = "SELECT CONCAT(`first_name`,' ',`last_name`) as `full_name` FROM `".$this->table_name."` WHERE `".$this->reset_token_column."` = :TOKEN";
        $st = $this->data_adapter->prepare($sql);
        $st->execute([
            ":TOKEN" => $token
        ]);
